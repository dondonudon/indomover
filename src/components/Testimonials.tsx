...
import { type GoogleReviewItem as GoogleReview, reviewsData as reviews } from "../lib/reviewsData";
...
function pickReviews(all: GoogleReview[]): GoogleReview[] {
	...
	const meaty = all.filter((r) => r.rating >= MIN_RATING && len(r) >= MIN_TEXT_LEN);
					...
					{hasReal && reviews.rating != null && (
						<div className="mt-5 flex items-center justify-center gap-3">
							<div className="flex items-center gap-1.5 rounded-full bg-white px-4 py-2 shadow-sm ring-1 ring-ink-200">
								<span className="text-xl font-bold text-ink-900">{reviews.rating?.toFixed(1)}</span>
					...
					<div className="mt-12 grid gap-6 md:grid-cols-3">
						{items.map((r, i) => {
							...
							return (
								...
								>
									<Quote className="absolute right-6 top-6 h-8 w-8 text-brand-100" aria-hidden />
									<div className="flex gap-0.5 text-amber-400">
										{Array.from({ length: 5 }).map((_, idx) => (
											<Star
												key={idx}
									...
									<figcaption className="mt-6 flex items-center gap-3 border-t border-ink-200 pt-4">
										...
										<div className="min-w-0">
											...
											{r.relativeTime && (
												<p className="truncate text-xs text-ink-700">{r.relativeTime}</p>
											)}
										</div>
									</figcaption>
								</motion.figure>
